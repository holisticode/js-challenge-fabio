...
/*
 * Linted, adapted and Updated to D3 v5 by Fabio Barone 2020.03.02
 */
...
export default function realTimeChartMulti() {
  ...
  var chart = function(s) {
    ...
    y = d3.scalePoint().domain(yDomain).range([height, 0])
    ...
    yNav = d3.scalePoint().domain(yDomain).range([heightNav, 0])
    ...
    setInterval(function() {
      ...
      var extent = (d3.event && d3.event.selection === null)? xNav.domain() : viewport.extent();
      //var interval = extent[1].getTime() - extent[0].getTime();
      var interval = x.invert(extent[1]) - x.invert(extent[0]);
      var offset = x.invert(extent[0])- x.invert(xNav.domain()[0]);
